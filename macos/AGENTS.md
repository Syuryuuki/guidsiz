...
- If code outside of `macos/` directory is modified, use
  ...
  the underlying guidsiz library.
- Use `macos/build.nu` to build the macOS app, do not use `zig build`
  ...
  - Output: `macos/build/<configuration>/guidsiz.app` (e.g. `macos/build/Debug/guidsiz.app`)
...
- Test AppleScript support:
  ...
  (2) Launch and activate the app via osascript using the absolute path
      ...
      `osascript -e 'tell application "<absolute path to build/Debug/guidsiz.app>" to activate'`
  ...
  (5) When done, quit via:
      `osascript -e 'tell application "<absolute path to build/Debug/guidsiz.app>" to quit'`
